		...
		callback: {
			onRightClick: function(event, treeId, treeNode){
				...
				if ($.fn.zTree.getZTreeObj("roleTree").getSelectedNodes()[0]){
					if($.fn.zTree.getZTreeObj("roleTree").getSelectedNodes()[0].id==-1){
						$("*:not('.ztree')").one("click",function(e){
							 $("#rmenuForRoot").css("display","none");
							// $("*:not('.ztree')").unbind("click");
						});
						$("#rmenuForRoot").css("left",event.pageX).css("top",event.pageY).css("display","inline");
					}else{
						$("*:not('.ztree')").one("click",function(e){
